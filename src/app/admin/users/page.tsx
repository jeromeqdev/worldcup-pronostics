...
export default function AdminUsersPage() {
  ...
  const toggleAdmin = async (userId: string, current: boolean) => {
    ...
    if (error) { toast.error("Erreur lors de la modification"); } else { toast.success(current ? "Droits admin retirés" : "Droits admin accordés"); fetchUsers(); }
  };
  ...
  const deleteUser = async (userId: string, pseudo: string) => {
    ...
    if (error) { toast.error("Erreur lors de la suppression"); } else { toast.success(`${pseudo} supprimé`); fetchUsers(); }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-4xl font-bold text-white tracking-wide">UTILISATEURS</h1>
        <p className="text-gray-500 text-sm">{users.length} inscrits</p>
      </div>
      <div className="card p-0 overflow-hidden">
        ...
        {users.map((u) => (
          <div key={u.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-3 px-4 py-3 border-b border-surface-700/50 last:border-0 items-center">
            ...
            <div>{u.is_admin ? <span className="badge-live text-xs">Admin</span> : <span className="badge-upcoming text-xs">Joueur</span>}</div>
            <div className="flex items-center gap-2">
              <button onClick={() => toggleAdmin(u.id, u.is_admin)} className="btn-ghost p-1.5" title={u.is_admin ? "Retirer admin" : "Donner admin"}>
                {u.is_admin ? <ShieldOff size={15} /> : <Shield size={15} />}
              </button>
              <button onClick={() => deleteUser(u.id, u.pseudo)} className="btn-ghost p-1.5 text-red-400 hover:text-red-300" title="Supprimer">
                <Trash2 size={15} />
              </button>
            </div>
          </div>
