...
export default function AdminResultsPage() {
  ...
  const fetchMatches = async () => {
    const { data } = await supabase.from("matches").select("*, home_team:teams!home_team_id(name, country_code), away_team:teams!away_team_id(name, country_code)").order("kickoff_time", { ascending: true });
    const list = (data ?? []) as MatchRow[];
    setMatches(list);
    const init: typeof editData = {};
    list.forEach((m) => { init[m.id] = { home: m.home_score?.toString() ?? "", away: m.away_score?.toString() ?? "", status: m.status }; });
    setEditData(init);
    setLoading(false);
  };
