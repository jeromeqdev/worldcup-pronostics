  ...
  const adminCards = [
    {
      ...
      desc: `${matchesCount ?? 0} matchs · ${finishedCount ?? 0} terminés`,
      color: "text-pitch-400",
      bg: "bg-pitch-900/30 border-pitch-700/40",
    },
    {
      href: "/admin/users",
      icon: Users,
      label: "Gérer les utilisateurs",
      desc: `${usersCount ?? 0} participants inscrits`,
      color: "text-blue-400",
    ...
    },
    {
      ...
      desc: `${(matchesCount ?? 0) - (finishedCount ?? 0)} matchs en attente`,
      color: "text-gold-400",
    ...
    },
    {
      ...
      icon: RefreshCw,
      label: "Recalculer classement",
      desc: `${predictionsCount ?? 0} pronostics au total`,
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {adminCards.map(({ href, icon: Icon, label, desc, color, bg }) => (
          <Link
            key={href}
            href={href}
            className={`card-hover border ${bg} flex items-center gap-4`}
          >
